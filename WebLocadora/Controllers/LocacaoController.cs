...
namespace WebLocadora.Controllers
{
    public class LocacaoController : Controller
    {
        ...
        {
            IEnumerable<Locacao> locacaos = null;
            ...
            using (var client = new HttpClient())
            {
                ...
                //HTTP GET
                var responseTask = client.GetAsync("Locacaos");
                responseTask.Wait();
                var result = responseTask.Result;

                if (result.IsSuccessStatusCode)
                {
                    var readTask = result.Content.ReadAsAsync<IList<Locacao>>();
                    readTask.Wait();
                    locacaos = readTask.Result;
                }
                else
                {
                    locacaos = Enumerable.Empty<Locacao>();
